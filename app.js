...
async function apiGet(action, params = {}){
  ...
  try{
    ...
    // _ts: evita que el navegador entregue una respuesta vieja del caché
    url.search = new URLSearchParams({ action, ...params, _ts: Date.now() }).toString();
...
/* Quita de CUENTAS_DATA la cuenta que se acaba de procesar y repinta.
   Es solo pantalla: la verdad la sigue trayendo cargarCuentasPendientes(). */
function quitarCuentaDeLista(c){
  if(!Array.isArray(CUENTAS_DATA)) return;
  const id  = String(c?.idCuenta || '').trim();
  const doc = String(c?.documento || '').trim();
  const inf = String(c?.informe || '').trim();

  CUENTAS_DATA = CUENTAS_DATA.filter(x => {
    const xid = String(x?.idCuenta || '').trim();
    if(id && xid) return xid !== id;
    return !(String(x?.documento || '').trim() === doc &&
             String(x?.informe   || '').trim() === inf);
  });
...
async function guardarOrdenPagoFlow(c){
  ...
  try{
    // El backend verifica que el estado quedó guardado; si no, lanza error
    // y aquí NO se envía WhatsApp ni se dice que se creó.
    ...
    // ✅ Quitar la tarjeta ya procesada de la lista en pantalla (por carpeta BH,
    //    y si no hay, por documento+informe). Luego se recarga desde la hoja.
    quitarCuentaDeLista(c);
